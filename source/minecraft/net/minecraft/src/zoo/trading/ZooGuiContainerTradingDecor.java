...
public class ZooGuiContainerTradingDecor extends GuiContainer
{
	...
	public ZooGuiContainerTradingDecor(EntityPlayer player)
	{
		super(new ZooContainerTradingDecor(player));
	...
	{
		Trade.handleMouseClick(slot, mc, inventorySlots, inventory, j, flag);
	}
	...
	public void drawScreen(int i, int j, float f)
	{
		...
		if (isScrolling)
		{
			...
			((ZooContainerTradingDecor) inventorySlots).scrollTo(currentScroll);
		}
	...
	public void handleMouseInput()
	{
		...
		if (i != 0 && ((ZooContainerTradingDecor) inventorySlots).itemList.size() > 40)
		{
			int j = (((ZooContainerTradingDecor) inventorySlots).itemList.size() / 8 - 5) + 1;
			if (i > 0)
			{
				i = 1;
			}
