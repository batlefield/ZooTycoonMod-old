...
public class ZooContainerTradingSpecial extends Container
{

    private static Minecraft minecraft = ModLoader.getMinecraftInstance();
    public static ZooContainerTradingSpecial instance = new ZooContainerTradingSpecial();
    public static boolean interact;
    public static List itemList;

    public ZooContainerTradingSpecial()
    {
        loadStacks();
    }
    ...
    public void loadStacks()
    {
        ...
        Item aitem[] = {
            Item.ingotGold, Item.ingotIron, Item.record11, Item.record13, Item.recordBlocks, Item.recordCat, Item.recordChirp, Item.recordFar, Item.recordMall, Item.recordMellohi, Item.recordStal, 
            Item.recordStrad, Item.recordWard, Item.saddle
        };
        ...
        for(int l2 = 0; l2 < 5; l2++)
        {
            ...
            {
                addSlot(new Slot(ZooGuiContainerTradingSpecial.getInventory(), k3 + l2 * 8, 8 + k3 * 18, 18 + l2 * 18));
            }
    ...
    public void scrollTo(float f)
    {
        ...
        for(int k = 0; k < 9; k++)
        {
            for(int l = 0; l < 8; l++)
            {
                ...
                {
                    ZooGuiContainerTradingSpecial.getInventory().setInventorySlotContents(l + k * 8, (ItemStack)itemList.get(i1));
                } else
                {
