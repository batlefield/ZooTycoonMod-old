...
public class mod_ZooTrade extends NetworkMod {
	...
	public String getVersion() {
		return new StringBuilder().append("Zoo Tycoon ")
				.append(mod_ZooCore.version).append(" Trading for Minecraft ")
				.append(mod_ZooCore.mcVersion).append(" by ")
				.append(mod_ZooCore.author).toString();
	}
	...
	static Configuration config = new Configuration(new File(
			Minecraft.getMinecraftDir(), "Zoo/Trade/Config.cfg"));

	private static int getBlockID(String s, int i) {
		config.load();
		config.getOrCreateBlockIdProperty(s, i);
		config.save();
		return new Integer(config.getOrCreateBlockIdProperty(s, i).value)
				.intValue();
	}
	...
	private static int getItemID(String s, int i) {
		...
		config.save();
		return new Integer(config.getOrCreateIntProperty(s,
				config.CATEGORY_ITEM, i).value).intValue();
	}
	...
	public boolean onTickInGame(float f, Minecraft minecraft1) {
		ScaledResolution scaledresolution = new ScaledResolution(
				minecraft1.gameSettings, minecraft1.displayWidth,
				minecraft1.displayHeight);
		...
		if (money < 0) {
			s = (new StringBuilder()).append("§4Money balance: $")
					.append(money).toString();
		} else if (money >= 100000) {
			s = (new StringBuilder()).append("§aMoney balance: $")
					.append(money).toString();
		} else {
			s = (new StringBuilder()).append("Money balance: $").append(money)
					.toString();
		}

		minecraft1.fontRenderer
				.drawStringWithShadow(
						s,
						k
								- (flag ? minecraft1.fontRenderer
										.getStringWidth(s) : 0), l, -1);
		l += byte0;
	...
	public boolean onTickInGUI(float f, Minecraft minecraft1,
			GuiScreen guiscreen) {
	...
	public static void debug(int i) {
		if (debug) {
			minecraft.thePlayer.addChatMessage(new String((new StringBuilder())
					.append("Debug Info: ").append(i)));
		}
	}
	...
	public static void loadmod() {
		...
		ModLoader.registerEntityID(ZooEntityShopKeeper.class, "Shop keeper",
				ModLoader.getUniqueEntityId());

		ModLoader
				.addRecipe(
						new ItemStack(shopkeeperblockdouble, 1),
						new Object[] { "XXX", "YYY", "XXX",
								Character.valueOf('X'), Block.planks,
								Character.valueOf('Y'), Block.thinGlass });
	}

	public static Item Coin = (new ZooItemCoin(getItemID("Coin", 407))
			.setItemName("sc").setIconIndex(9));
	public static final Block shopkeeperblockdouble = (new BlockShop(
			getBlockID("Shopkeeper block", 225))
			.setBlockName("shopkeeperdouble"));
